...
class RegisterHandler(BaseHandler):
    ...
    def post(self):
        ...
        if "2468" != sms_code:
            ...
            try:
                self.redis().detete("sms_code_%s" % mobile)
            except Exception as e:
                logging.error(e)

        # 保存数据，同时判断手机号是否存在,判断的依据
...
class LoginHandler(BaseHandler):
    ...
    def post(self):
        ...
        # 检查密码是否是正确
        res = self.db.get("select up_user_id,up_name,up_passwd from ih_user_profile where up_mobole=%(mobile)s",
                          mobile=mobile)
        ...
        if res and res["up_passwd"] == unicode(password):
            ...
            return self.write(dict(errcode=RET.DATAERR, errmsg="手机号码或者密码错误"))
